...
# s1) Give permission to the script
# chmod 777 Resource_tracker.sh
# s2) open the crontab to make cron entry to shecdule when will script will run
# crontab -e
# s3) Add Cron Entry
# Run every day at 9:00 AM
# 0 9 * * * /home/ubuntu/Resource_tracker.sh
# s4) Save and exit the crontab file
# s5) To check the cronjob is added or not
# crontab -l
# s6) check cron logs to verify the script is running as expected
# sudo cat /var/log/cron  (for amazon linux)